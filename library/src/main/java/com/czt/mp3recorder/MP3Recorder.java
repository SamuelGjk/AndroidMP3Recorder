...
public class MP3Recorder {
    ...
    private boolean mIsRunning = false; // 控制 start 和 stop
    private boolean mIsRecording = false; // 控制 resume 和 pause
    ...
    public void start() throws IOException {
        ...
        mIsRunning = true; // 提早，防止init或startRecording被多次调用
        initAudioRecorder();
        mIsRecording = true;
        recordThread = new Thread() {
            ...
            public void run() {
                ...
                boolean oldIsRecording = false;

                while (mIsRunning) {
                    if (mIsRecording != oldIsRecording) {
                        if (mIsRecording) {
                            mAudioRecord.startRecording();
                        } else {
                            mAudioRecord.stop();
                        }
                        oldIsRecording = mIsRecording;
                    }
                    if (mIsRecording) {
